...
export default function Shop() {
  ...
  const categories = ['All', 'Suits', 'Blazers', 'Formal Shirts', 'Trousers', 'Shirts', 'Accessories', 'Ethnic', 'Casual', 'Jeans'];
  ...
  useEffect(() => {
    async function fetchProducts() {
      ...
      try {
        ...
        if (category !== 'All') {
          firestoreQuery = query(q, where('category', '==', category));
        } else {
        ...
        // Frontend filtering for search (Firestore search is limited)
        ...
        <aside className="w-full lg:w-64 shrink-0">
          <div className="lg:sticky lg:top-32 space-y-8 md:space-y-10">
            ...
            <div className="p-6 bg-black text-white rounded-3xl hidden lg:block">
              ...
              <p className="text-xs text-white/50 mb-6 leading-relaxed">Up to 40% off on selected wedding suits and ethnic collections.</p>
              <Link to="/shop?category=Suits" className="block w-full text-center bg-white text-black py-3 rounded-full text-[10px] font-bold uppercase tracking-widest hover:bg-gray-200 transition-all">Explore Sale</Link>
            </div>
          </div>
        </aside>
